...
function AddMenuDiversMenu(menu)
	...
	local Description1 = "Dormir ou Se Reveiller"
	
	
    local ragdollItem = NativeUI.CreateItem(('Dormir | Se Reveiller'), Description1)
...
function AddSubMenuActionMenu(menu)
	animSaluteMenu = _menuPool:AddSubMenu(menu.SubMenu, _U('animation_action_title'))

	local saluerItem = NativeUI.CreateItem(_U('animation_action_saluate'), '')
	animSaluteMenu.SubMenu:AddItem(saluerItem)
	local serrerItem = NativeUI.CreateItem(_U('animation_action_serrer'), '')
	animSaluteMenu.SubMenu:AddItem(serrerItem)
	local tchekItem = NativeUI.CreateItem(_U('animation_action_tchek'), '')
	animSaluteMenu.SubMenu:AddItem(tchekItem)
	local banditItem = NativeUI.CreateItem(_U('animation_action_bandit'), '')
	animSaluteMenu.SubMenu:AddItem(banditItem)
	local militaryItem = NativeUI.CreateItem(_U('animation_action_military'), '')
	animSaluteMenu.SubMenu:AddItem(militaryItem)
	local suspectItem = NativeUI.CreateItem(_U('animation_action_suspect'), '')
	animSaluteMenu.SubMenu:AddItem(suspectItem)

	animSaluteMenu.SubMenu.OnItemSelect = function(sender, item, index)
		if item == saluerItem then
			startAnim('combat@chg_positionpose_b', 'aimb_calm_bwd')
		elseif item == serrerItem then
			startAnim('combat@chg_positionpose_b', 'aimb_calm_bwd_l')
		elseif item == tchekItem then
			startAnim('combat@chg_positionpose_b', 'aimb_dodgy_bwd_l')
		elseif item == banditItem then
			startAnim('combat@chg_positionpose_c', 'aimc_dodgy_bwd_l')
		elseif item == militaryItem then
			startAnim('combat@combat_reactions@pistol_1h_hillbilly', '180')
		elseif item == suspectItem then
			startAnim('combat@combat_reactions@pistol_1h_gang', '0')
